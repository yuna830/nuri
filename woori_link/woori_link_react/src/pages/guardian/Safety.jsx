...
const emptyForm = { seniorId: '', productType: '', productName: '', brandName: '', manufacturer: '', modelNumber: '', barcode: '', certificationNumber: '', manufacturingDate: '', currentUseStatus: 'IN_USE' };
...
export default function Safety() {
    ...
    try {
      ...
      if (analysis?.analysisId) {
        await confirmProductLabelAnalysis(
          analysis.analysisId,
          { productName: form.productName, brandName: form.brandName, manufacturer: form.manufacturer, modelNumber: form.modelNumber, barcode: form.barcode, certificationNumber: form.certificationNumber, manufacturingDate: form.manufacturingDate },
          response.data?.id,
        ).catch(() => {});
      }
  ...
  async function analyzeLabel() {
    ...
    try {
      ...
      setError(e.response?.data?.detail || '제품 라벨 OCR 분석에 실패했습니다. 직접 입력으로 등록할 수 있습니다.');
  ...
  return <GuardianLayout activeMenu="safety"><main className="guardian-safety-page">
    ...
    <section className="guardian-safety-section"><div className="guardian-safety-section__heading"><div><h2>등록 제품</h2><p>제품별 리콜 여부와 현재 사용 상태를 확인하세요.</p></div><span>{visibleProducts.length}개</span></div>
      {loading ? <div className="guardian-safety-empty">불러오는 중입니다.</div> : visibleProducts.length === 0 ? <div className="guardian-safety-empty">등록된 제품이 없습니다.</div> : <div className="guardian-product-grid">{visibleProducts.map(product => { const confirmed = product.recallDecisionStatus === 'RECALL_CONFIRMED' || (!product.recallDecisionStatus && product.recallStatus === 'RECALLED'); const evidence = matchedEvidence(product); const action = actionUi(product); return <article className={`guardian-product-card ${confirmed ? 'danger' : ''}`} key={product.id}><div className="guardian-product-card__top"><div><h3>{product.productName}</h3><p>{product.seniorName} 어르신</p></div><b>{product.recallCheckStatus === 'FAILED' ? '조회 실패' : confirmed ? `공식 리콜 대상 · ${action.status}` : RECALL_LABEL[product.recallDecisionStatus] || '확인 필요'}</b></div><dl>{product.brandName && <div><dt>브랜드</dt><dd>{product.brandName}</dd></div>}{product.modelNumber && <div><dt>모델번호</dt><dd>{product.modelNumber}</dd></div>}<div><dt>사용 상태</dt><dd>{USE_LABEL[product.currentUseStatus] || '확인 필요'}</dd></div><div><dt>최근 조회</dt><dd>{formatDate(product.lastSuccessfulCheckedAt || product.lastCheckedAt)}</dd></div></dl>{confirmed && <div className="guardian-product-card__alert"><strong>공식 리콜 대상 제품입니다.</strong>{product.hazardDescription && <span>{product.hazardDescription}</span>}<span>{product.consumerAction || action.fallback}</span>{evidence.length > 0 && <small>{evidence.join('와 ')}가 공식 공고와 일치했습니다.</small>}</div>}{product.stopGuidanceCompleted && <div className="guardian-product-card__completion"><strong>사용 중지 확인 완료</strong><span>{formatDate(product.stopGuidanceCompletedAt)} · 보호자 {product.stopGuidanceTarget || ''}</span></div>}<div className="guardian-product-card__actions"><button onClick={() => setDetail(product)}>리콜 상세 보기</button>{confirmed && <button className="primary guidance-action" onClick={() => setGuidanceTarget(product)}>{action.button}</button>}<details className="product-more"><summary aria-label="더 보기">⋮</summary><button className="delete" onClick={() => removeProduct(product)}>등록 삭제</button></details></div></article>; })}</div>}
    </section>
    ...
    {detail && (() => { const evidence = matchedEvidence(detail); const missing = asArray(detail.missingFields).map(field => MISSING_LABEL[field]).filter(Boolean); const confirmed = detail.recallDecisionStatus === 'RECALL_CONFIRMED'; const action = actionUi(detail); return <div className="guardian-safety-modal-backdrop" onMouseDown={e => e.target === e.currentTarget && setDetail(null)}><section className="guardian-safety-modal recall-detail-modal"><header><h2>리콜 상세</h2><button onClick={() => setDetail(null)}>×</button></header>{confirmed && <span className="recall-status-badge">공식 리콜 대상 · {action.status}</span>}<h3>{detail.productName}</h3><p className="recall-product-meta">{detail.seniorName} 어르신 · {USE_LABEL[detail.currentUseStatus] || '사용 상태 확인 필요'}{detail.modelNumber && <><br />모델번호 {detail.modelNumber}</>}</p>{detail.matchedRecallNotice?.imageUrls?.length > 0 && <section className="recall-photo-section"><strong>공식 제품 사진</strong><p>등록 제품이 아래 공식 리콜 제품과 같은지 추가로 확인해 주세요. 사진은 판정 근거가 아닌 확인 보조 자료입니다.</p><div className="recall-images">{detail.matchedRecallNotice.imageUrls.map(url => <img key={url} src={url} alt="공식 리콜 제품" />)}</div></section>}{evidence.length > 0 && <section className="recall-evidence"><strong>공식 공고와 일치한 정보</strong>{evidence.map(item => <span key={item}>✓ {item}</span>)}</section>}{missing.length > 0 && <section className="recall-missing"><strong>추가 확인 필요</strong><ul>{missing.map(item => <li key={item}>{item}</li>)}</ul></section>}<section className="recall-action-card"><strong>지금 해야 할 조치</strong><p>{detail.consumerAction || action.fallback}</p>{detail.inquiryTel && <b>문의처 {detail.inquiryTel}</b>}</section><section className="recall-description"><div><strong>제품 결함</strong><p>{detail.defectDescription || '공식 공고에서 확인해 주세요.'}</p></div><div><strong>위해 정보</strong><p>{detail.hazardDescription || '공식 공고에서 확인해 주세요.'}</p></div></section><dl className="recall-meta-list"><div><dt>공표일</dt><dd>{formatDate(detail.publishDate)}</dd></div><div><dt>최근 리콜 조회</dt><dd>{formatDate(detail.lastSuccessfulCheckedAt || detail.lastCheckedAt)}</dd></div><div><dt>공식 출처</dt><dd>{detail.sourceUrl ? <a href={detail.sourceUrl} target="_blank" rel="noreferrer">제품안전정보센터</a> : '-'}</dd></div></dl>{detail.stopGuidanceCompleted && <div className="guardian-product-card__completion"><strong>사용 중지 확인 완료</strong><span>{formatDate(detail.stopGuidanceCompletedAt)} · 보호자 {detail.stopGuidanceTarget || ''}</span></div>}{confirmed && <button className="submit guidance-action" onClick={() => setGuidanceTarget(detail)}>{action.button}</button>}</section></div>; })()}
