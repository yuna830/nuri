  ...
  Widget build(BuildContext context) {
    return ListView(
        ...
        children: [
          ...
          _navItem(icon: Icons.people_outline, label: '보호 대상자 목록',
              onTap: () => _go(const SeniorListScreen())),
          _divider(),
          _navItem(icon: Icons.person_add_outlined, label: '보호 대상자 추가',
              onTap: () => _go(const AddSeniorScreen())),
          _divider(),
          _navItem(icon: Icons.assignment_outlined, label: '대상자 정보 동의 관리',
              onTap: () => _go(const SeniorConsentScreen())),
          const SizedBox(height: 24),

          // ── 섹션 2: 정보 제공 동의 ───────────────────────────
          _sectionLabel('정보 제공 동의'),
          const SizedBox(height: 6),
          _toggleItem(
            icon: Icons.location_on_outlined,
            label: '위치 정보 사용',
            desc: '마지막 위치와 안전 구역 정보를 확인합니다.',
            key: ConsentStorage.locationKey, def: true,
          ),
          _divider(),
          _toggleItem(
            ...
            key: ConsentStorage.contactKey, def: true,
          ),
          _divider(),
          _toggleItem(
            icon: Icons.favorite_outline,
            label: '건강 정보 포함',
            desc: '신고 정보 복사 시 질병/복약 정보를 포함합니다.',
            key: ConsentStorage.healthKey, def: false,
          ),
          _divider(),
          _toggleItem(
            icon: Icons.copy_outlined,
            label: '신고 정보 복사 허용',
            desc: '안전Dream 신고용 참고 정보를 클립보드에 복사합니다.',
            key: ConsentStorage.reportCopyKey, def: true,
          ),
          const SizedBox(height: 24),
          ...
          _sectionLabel('앱 설정'),
          const SizedBox(height: 6),
          _navItem(icon: Icons.notifications_outlined, label: '알림 설정',
              onTap: () => _go(const NotificationSettingsScreen())),
          _divider(),
          _navItem(icon: Icons.privacy_tip_outlined, label: '개인정보 처리 안내',
