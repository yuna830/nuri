...
List<_ConsentItem> _defaultItems() => [
      _ConsentItem(
        key: 'location',
        label: '위치 정보',
        desc: '마지막 위치와 이동 정보를 보호자가 확인합니다.',
                          ...
                          Padding(
                            ...
                            child: Text(
                              ...
                              style: const TextStyle(
                                  fontSize: 13, color: _kSub, height: 1.5),
                            ),
                          ),
                          for (int i = 0; i < _seniors.length; i++) ...[
                            if (i > 0)
                              const Divider(
                                  height: 1, color: _kDiv),
                            _SeniorRow(senior: _seniors[i]),
                          ],
                        ],
                      ),
                    ),
...
class _SeniorRow extends StatelessWidget {
  final Senior senior;
  const _SeniorRow({required this.senior});
    ...
    return ColoredBox(
      ...
      child: ListTile(
        ...
        onTap: () => Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => SeniorConsentDetailScreen(senior: senior),
          ),
        ).then((_) {
          // 복귀 시 목록 갱신
...
class SeniorConsentDetailScreen extends StatefulWidget {
  final Senior senior;
  const SeniorConsentDetailScreen({super.key, required this.senior});
...
class _SeniorConsentDetailScreenState
  ...
  Future<void> _requestAll() async {
    // TODO: 백엔드 연동 시 POST /api/consent/request 호출
    setState(() {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('동의 요청을 전송했습니다. 어르신이 수락하면 허용됩니다.')),
      );
    }
  }
            ...
            child: ListView(
              ...
              children: [
                ...
                for (int i = 0; i < _items.length; i++) ...[
                  if (i > 0)
                    const Divider(
                        height: 1, indent: 56, color: _kDiv),
                ...
                Padding(
                  ...
                  child: Text(
                    ...
                    style: const TextStyle(
                        fontSize: 12, color: _kHint, height: 1.6),
                  ),
                ),
              ],
            ),
