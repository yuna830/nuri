...
import joblib
import pandas as pd
from health_features import REQUIRED_COLUMNS, build_features, load_rows, normalize_label, validate_columns
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a sklearn health status model.")
    parser.add_argument("--input", default="data/nhanes_health_status_training.csv", help="Labeled CSV or JSON file.")
...
def main() -> None:
    ...
    joblib.dump(model, output_dir / "health_status_model.joblib")
    (output_dir / "feature_columns.json").write_text(json.dumps(list(x.columns), ensure_ascii=False, indent=2), encoding="utf-8")
        ...
        json.dumps(
            {
                ...
                "model": str(output_dir / "health_status_model.joblib"),
