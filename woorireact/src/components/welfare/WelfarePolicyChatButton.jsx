...
function getEvidenceDisplay(item) {
    ...
    return {
        organization: "복지 자료",
        title: item.service_name || item.filename?.replace(/\.md$/i, "").replace(/\.pdf$/i, "") || "참고 문서",
    };
}
...
function WelfarePolicyChatButton({ senior = null, seniorOptions = [], variant = "floating" }) {
    ...
    const normalizeNameText = (value) =>
        String(value || "")
            .replace(/\s+/g, "")
            .replace(/님|사용자|어르신|대상자|회원|보호대상자/g, "")
            .trim();
    ...
    const resolveSeniorForQuestion = (trimmedQuestion) => {
        ...
        const normalizedQuestion = normalizeNameText(trimmedQuestion);

        return availableSeniors.find((item) => {
            const name = normalizeNameText(item.name);
    ...
    }, [selectedSenior?.id, selectedSenior?.name]);
...
export default WelfarePolicyChatButton;
