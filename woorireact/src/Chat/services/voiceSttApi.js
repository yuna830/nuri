//const DEFAULT_STT_SERVER_HOST = "172.28.6.243";
const DEFAULT_STT_SERVER_HOST = "192.168.219.113";

const STT_SERVER_HOST =
...
function normalizeServerUrl(value, port, path) {
  ...
  if (!rawValue) {
    return `http://${STT_SERVER_HOST}:${port}${path}`;
  }

  if (rawValue.startsWith(":")) {
    return `http://${STT_SERVER_HOST}${rawValue}`;
  }

  if (rawValue.startsWith("/")) {
    return `http://${STT_SERVER_HOST}:${port}${rawValue}`;
  }
