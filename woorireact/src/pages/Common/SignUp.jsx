...
export default function SignUp() {
  ...
  const submit = async () => {
    try {
      ...
      setError("회원가입 저장에 실패했습니다. 서버가 켜져 있는지 확인해주세요.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="su-root">
      <nav className="su-nav">
        ...
        <div className="su-nav-step">사용자 정보 등록 {step + 1} / {STEPS.length}</div>
      </nav>

      <div className="su-stepbar">
        <div className="su-stepbar-inner">
          {STEPS.map((stepLabel, index) => (
            <div key={stepLabel} className={`su-step-item ${index < STEPS.length - 1 ? "grow" : ""}`}>
              <div className={`su-step-circle ${index < step ? "done" : index === step ? "active" : ""}`}>
                {index < step ? "✓" : index + 1}
              </div>
              <span className={`su-step-label ${index === step ? "active" : ""}`}>{stepLabel}</span>
              {index < STEPS.length - 1 && <div className={`su-step-line ${index < step ? "done" : ""}`} />}
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="su-layout">
          ...
          <section className="su-section">
            ...
            <div className="su-photo-row">
              ...
              <div className="su-photo-actions">
                ...
                {form.profileImageUrl && (
                  <button className="su-photo-remove" type="button" onClick={() => set("profileImageUrl", "")}>사진 삭제</button>
                )}
              </div>
            </div>
            ...
            <div className="su-field">
              <label className="su-label">거주지 <span className="su-required">*</span></label>
              <input className="su-input" value={form.region} onChange={(event) => set("region", event.target.value)} placeholder="예: 서울특별시 송파구" />
            </div>

            <div className="su-field">
              <label className="su-label">전화번호 <span className="su-required">*</span></label>
              <input className="su-input" value={form.phone} onChange={(event) => set("phone", formatPhoneNumber(event.target.value))} placeholder="예: 010-0000-0000" />
            </div>
            ...
            <div className="su-row">
              <div className="su-field">
                ...
                <select className="su-select" value={form.disabilityGrade} onChange={(event) => set("disabilityGrade", event.target.value)}>
                  {DISABILITY_GRADES.map((value) => <option key={value} value={value}>{value}</option>)}
                </select>
              </div>
              <div className="su-field">
                ...
                <select className="su-select" value={form.disabilityType} onChange={(event) => set("disabilityType", event.target.value)}>
                  {DISABILITY_TYPES.map((value) => <option key={value} value={value}>{value}</option>)}
                </select>
              </div>
            </div>
          </section>
        ...
        {step === 1 && (
          <>
            <section className="su-section">
              ...
              <div className="su-row">
                <div className="su-field">
                  <label className="su-label">키(cm) <span className="su-required">*</span></label>
                  <input className="su-input" type="number" value={form.height} onChange={(event) => set("height", event.target.value)} placeholder="예: 160" />
                </div>
                <div className="su-field">
                  <label className="su-label">몸무게(kg) <span className="su-required">*</span></label>
                  <input className="su-input" type="number" value={form.weight} onChange={(event) => set("weight", event.target.value)} placeholder="예: 58" />
                </div>
              </div>
              ...
              {bmi && (
                <div className="su-bmi-box">
                  ...
                  <div className="su-bmi-guide">
                    {form.age}세 · {form.gender || "성별 미선택"} 기준 입력값으로 계산<br />
                    BMI = 몸무게(kg) / 키(m)의 제곱
                  </div>
                </div>
              )}
                ...
                <div className="su-field">
                  ...
                  <div className="su-check-group">
                    {["없음", "과거 흡연", "흡연 중"].map((value) => (
                      <button key={value} className={`su-chip ${form.smoking === value ? "on" : ""}`} type="button" onClick={() => set("smoking", value)}>{value}</button>
                    ))}
                  </div>
                </div>
                <div className="su-field">
                  ...
                  <div className="su-check-group">
                    {["없음", "가끔", "자주"].map((value) => (
                      <button key={value} className={`su-chip ${form.drinking === value ? "on" : ""}`} type="button" onClick={() => set("drinking", value)}>{value}</button>
                    ))}
                  </div>
                </div>
              </div>
              ...
              <div className="su-field">
                ...
                <div className="su-check-group">
                  {["없음", "1~2개", "3~5개", "6개 이상"].map((value) => (
                    <button key={value} className={`su-chip ${form.medicineCount === value ? "on" : ""}`} type="button" onClick={() => set("medicineCount", value)}>{value}</button>
                  ))}
                </div>
              </div>
            </section>
            ...
            <section className="su-section">
              ...
              <div className="su-hint">정확히 모르시면 가장 가까운 정도를 선택해주세요. 나중에 내 정보 수정에서 다시 바꿀 수 있어요.</div>
                ...
                <div className="su-disease-row" key={key}>
                  ...
                  <div className="su-check-group">
                    {SIMPLE_LEVELS.map((level) => (
                      <button key={level} className={`su-chip ${form[key] === level ? "on" : ""}`} type="button" onClick={() => set(key, level)}>{level}</button>
                    ))}
                  </div>
                </div>
              ))}
            ...
            <section className="su-section">
              ...
              {[
                ...
                <div className="su-disease-row" key={key}>
                  ...
                  <div className="su-check-group">
                    {options.map((value) => (
                      <button key={value} className={`su-chip ${form[key] === value ? "on" : ""}`} type="button" onClick={() => set(key, value)}>{value}</button>
                    ))}
                  </div>
                </div>
              ))}
              ...
              {form.hasSurgery === "있음" && (
                <div className="su-field">
                  ...
                  <textarea className="su-input su-textarea" value={form.surgeryDetail} onChange={(event) => set("surgeryDetail", event.target.value)} placeholder="예: 2023년 무릎 수술" rows={3} />
                </div>
              )}

              <div className="su-field">
                <label className="su-label">기타 건강 참고사항</label>
                <textarea className="su-input su-textarea" value={form.otherDisease} onChange={(event) => set("otherDisease", event.target.value)} placeholder="예: 오래 서 있으면 허리가 아픔" rows={3} />
              </div>
            </section>
          </>
        )}
        ...
        {step === 2 && (
          <section className="su-section">
            ...
            <div className="su-field">
              ...
              <div className="su-check-group">
                {WORK_TYPES.map((workType) => (
                  <button key={workType} className={`su-chip ${(form.disabledWork || []).includes(workType) ? "on" : ""}`} type="button" onClick={() => toggleArr("disabledWork", workType)}>{workType}</button>
                ))}
              </div>
            </div>
            ...
            <div className="su-field">
              ...
              <div className="su-check-group">
                {["무관", "시급", "월급", "일당"].map((value) => (
                  <button key={value} className={`su-chip ${form.payType === value ? "on" : ""}`} type="button" onClick={() => set("payType", value)}>{value}</button>
                ))}
              </div>
            </div>
            ...
            <div className="su-field">
              ...
              <div className="su-check-group">
                {DAYS.map((day) => (
                  <button key={day} className={`su-chip ${(form.hopeDays || []).includes(day) ? "on" : ""}`} type="button" onClick={() => toggleArr("hopeDays", day)}>{day}</button>
                ))}
              </div>
            </div>
            ...
            <div className="su-field">
              ...
              <div className="su-check-group">
                {JOB_TYPES.map((value) => (
                  <button key={value} className={`su-chip ${(form.hopeJobType || []).includes(value) ? "on" : ""}`} type="button" onClick={() => toggleArr("hopeJobType", value)}>{value}</button>
                ))}
              </div>
            </div>
            ...
            <div className="su-field">
              ...
              <div className="su-check-group">
                {JOB_CONDITIONS.map((value) => (
                  <button key={value} className={`su-chip ${(form.hopeCondition || []).includes(value) ? "on" : ""}`} type="button" onClick={() => toggleArr("hopeCondition", value)}>{value}</button>
                ))}
              </div>
            </div>
            ...
            <div className="su-field">
              ...
              <textarea className="su-input su-textarea" value={form.memo} onChange={(event) => set("memo", event.target.value)} placeholder="예: 앉아서 할 수 있는 일을 선호함" rows={4} />
            </div>
          </section>
        )}
        ...
        <div className="su-btn-row">
          {step > 0 ? (
            <button className="su-btn-prev" type="button" onClick={() => { setError(""); setStep(step - 1); window.scrollTo(0, 0); }}>← 이전</button>
          ) : (
            <button className="su-btn-prev" type="button" onClick={() => navigate("/")}>로그인으로</button>
          )}
