...
int _matchScore(
    Map<String, dynamic> job, Map<String, dynamic> profile, String category) {
  int score = 0;
  final jobText =
      '${job['recrtTitle'] ?? ''} ${job['jobclsNm'] ?? ''} ${job['workPlcNm'] ?? ''}'
          .toLowerCase();
...
class _JobScreenState extends State<JobScreen> {
  ...
  int? get _jobAccessAge => jobAccessAgeFromProfile(_profile);
  ...
  Future<void> _loadAll() async {
    ...
    await Future.wait([_loadJobs(replace: true), _loadApplications()]);
  }
  ...
  Future<void> _loadApplications() async {
    if (_profile.isNotEmpty && !_canAccessJobs) return;
    try {
      final list = await _api.fetchJobApplications(widget.seniorId);
      if (mounted) {
        setState(() =>
            _applications = list.whereType<Map<String, dynamic>>().toList());
      }
    } catch (_) {}
  }
  ...
  Widget build(BuildContext context) {
    ...
    final recommendedKeys =
        recommended.map((j) => '${j['source']}-${j['jobId']}').toSet();
    final listed = scored
        .where((j) => !recommendedKeys.contains('${j['source']}-${j['jobId']}'))
        .toList();
    ...
    return Scaffold(
      ...
      appBar: widget.hideAppBar
          ...
          : AppBar(
              ...
              actions: [
                if (_canAccessJobs && applied.isNotEmpty)
                  TextButton.icon(
                  ...
                  if (_error != null)
                    Padding(
                      ...
                      child: Container(
                        ...
                        child: Text('⚠️ $_error',
                            style: const TextStyle(
                                color: Color(0xFFD94E4E), fontSize: 13)),
                      ),
                    ),
                  Expanded(
                      ...
                      child: CustomScrollView(
                        slivers: [
                          ...
                          if (welfareRecommended.isNotEmpty)
                            SliverToBoxAdapter(
                              child: _WelfareRecommendSection(
                                  applications: welfareRecommended),
                            ),
                          // 내 프로필 조건
...
class _CategoryChipsState extends State<_CategoryChips> {
  ...
  void _shift(int direction) {
    final currentIndex =
        _categories.indexWhere((c) => c.value == widget.selected);
    final nextIndex =
        (currentIndex + direction).clamp(0, _categories.length - 1);
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(12),
        border:
            Border.all(color: const Color(0xFFF0B429).withValues(alpha: 0.4)),
      ),
        ...
        ...applications.take(3).map((a) => Padding(
              ...
              child: Row(children: [
                ...
                Text(
                  ...
                  style:
                      const TextStyle(color: Color(0xFF78350F), fontSize: 12),
                ),
              ]),
            )),
      ]),
...
class _ProfileConditionCardState extends State<_ProfileConditionCard> {
  ...
  Widget build(BuildContext context) {
    ...
    if ((p['payType'] as String? ?? '').isNotEmpty)
      basic['급여형태'] = p['payType'];
    if ((p['maxHours'] as String? ?? '').isNotEmpty)
      basic['활동시간'] = p['maxHours'];
    if ((p['maxDistance'] as String? ?? '').isNotEmpty)
      basic['이동거리'] = p['maxDistance'];

    // 더보기 조건
    final extra = <String, String>{};
    final hopeDays = p['hopeDays'];
    if (hopeDays is List && hopeDays.isNotEmpty)
      extra['희망요일'] = _listStr(hopeDays);
    final hopeJobType = p['hopeJobType'];
    if (hopeJobType is List && hopeJobType.isNotEmpty)
      extra['희망직종'] = _listStr(hopeJobType);
    final hopeCondition = p['hopeCondition'];
    if (hopeCondition is List && hopeCondition.isNotEmpty)
      extra['희망조건'] = _listStr(hopeCondition);
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(12),
        border:
            Border.all(color: const Color(0xFF86A788).withValues(alpha: 0.3)),
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          const Text('내 희망 조건',
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: expired
              ...
              : [
                  BoxShadow(
                      color: const Color(0xFFF0B429).withValues(alpha: 0.12),
                      blurRadius: 8,
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(
            ...
            style: TextStyle(
                color:
                    expired ? const Color(0xFF9CA3AF) : const Color(0xFF1F2A20),
                fontSize: 13,
                fontWeight: FontWeight.w800),
            maxLines: 2,
            overflow: TextOverflow.ellipsis,
...
class _JobCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final expired = _isExpired(job);
    final empl =
        _emplMap[job['emplymShp']] ?? job['emplymShpNm']?.toString() ?? '기타';
    final category = _categorize(job);
    final score = job['__score'] as int? ?? 0;

    // 색깔: 마감=회색, 일반=초록
    final barColor =
        expired ? const Color(0xFFD1D5DB) : const Color(0xFF86A788);
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: expired
              ...
              : [
                  BoxShadow(
                      color: const Color(0xFF86A788).withValues(alpha: 0.06),
                      blurRadius: 8,
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(
            ...
            style: TextStyle(
              color:
                  expired ? const Color(0xFFCCCCCC) : const Color(0xFF86A788),
              fontSize: 12,
...
class _JobDetailSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final rows = <_Row>[
      _Row('출처', job['source']),
      _Row('추천점수', '$score점'),
      _Row('고용형태', _emplMap[job['emplymShp']] ?? job['emplymShpNm']),
      _Row('근무지', job['workPlcNm']),
      _Row('상세주소', job['plDetAddr']),
      _Row('직종', job['jobclsNm']),
      _Row('근무시간', job['workTime']),
      _Row('주당시간', job['weekHours'] != null ? '${job['weekHours']}시간' : null),
      _Row('급여', job['wage']),
      _Row('모집인원', job['clltPrnnum'] != null ? '${job['clltPrnnum']}명' : null),
                ...
                child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        job['oranNm']?.toString() ?? '기업명 미공개',
                        style: const TextStyle(
                            color: Color(0xFF6D766A), fontSize: 13),
                      ),
                    ]),
              ),
              IconButton(
            ...
            child: applications.isEmpty
                ? const Center(
                    child: Text('아직 신청한 일자리가 없습니다',
                        style:
                            TextStyle(color: Color(0xFF6D766A), fontSize: 15)))
                : ListView.separated(
                    controller: ctrl,
                    ...
                    itemBuilder: (_, i) {
                      ...
                      final statusColor =
                          _statusColor(a['status']?.toString() ?? '');
                      return ListTile(
